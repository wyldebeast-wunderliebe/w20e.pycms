...
class XMLFormFactory(object):
    ...
    def createForm(self, form_name=None):
        form_path = os.path.join(
            ...
            "..",
            "forms",
            "%s.xml" % (form_name or self.context.content_type),
        )
...
class PyCMSMixin(object):
    ...
    def __json__(self, request):
        ...
        data = self._data_.as_dict()

        lexical = request.params.get("lexical")

        _form = self._form_(request)
        _form.data.update(self._data_)
...
class BaseFolder(PyCMSMixin, HitmanBaseFolder):
    ...
    def list_content(self, content_type=None, iface=None, **kwargs):
        ...
        if "order_by" not in kwargs:
            order_by = getattr(self, "order_by_attr", "created")
        ...
        result = HitmanBaseFolder.list_content(self, content_type, iface, **kwargs)
